function [RESULTS] = Second_BE_Final_SparseALGs_enh(img,batchSize,sparse_,dicType) %#ok<*INUSD>
...
%[Batches, batchPerRow,batchPerCol] = getIMGvectors(img_double,batchSize);
[VecMatrix,kr,kc] = getIMGvectors(img_double,batchSize);
index = 1;
...
warning('off','all');
y_init = zeros(batchSize,1);%zeros(batchSize^2,1);
supp_init =[];
omp_sp_sum = [];
eomp_sp_sum = [];
X_OMP = [];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        threshold = 0.6;
        [~, coeff_, iopt_omp, OMP_ERROR_,OMP_SSIM_, y_i_omp__]       = SSIM_OMP_new_pap5(Dic, Vector,sparse,threshold);
        [~, coeff, iopt_eomp, OMP_ERROR_2,OMP_SSIM_2, y_i_omp_n]       = Enh_Convergence_OMP_pap5(Dic, Vector,sparse,y_init,supp_init,threshold);
        if mod(k,9) == 0
           y_init = y_i_omp_n(:,end).*0;
        else
           y_init = y_i_omp_n(:,end);
        end
        supp_init = iopt;
        
        omp_sp_sum =   [omp_sp_sum;  size(iopt_omp,1) OMP_SSIM_(end,1)];
        eomp_sp_sum =  [eomp_sp_sum; size(iopt_eomp,1) OMP_SSIM_2(end,1)];
        omp_sp_sum_mean = [mean(omp_sp_sum(:,1)) mean(omp_sp_sum(:,2))];
        eomp_sp_sum_mean = [mean(eomp_sp_sum(:,1)) mean(eomp_sp_sum(:,2))];
        X_OMP = [X_OMP Vector];
        Xhat_OMP = [Xhat_OMP y_i_omp__(:,end)];
        X_eOMP = [X_eOMP Vector];
        Xhat_eOMP = [Xhat_eOMP y_i_omp_n(:,end)];


        
ALL = [ALL; size(iopt_omp,1) OMP_SSIM_(end,1) size(iopt_eomp,1) OMP_SSIM_2(end,1)];
        %fprintf(size(iopt_omp,1), size(iopt_eomp,1));%2012
       
        fprintf(strcat('Processing ' , img , ' by >>>' , ' (EOMP), %d\n'), k);%2012
        
        %compare_err = [OMP_ERROR_ OMP_ERROR_2]; 
        %compare_ssi = [OMP_SSIM_ OMP_SSIM_2];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
CCC = 0;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        EOMP_values = ALL;
        scatter([ALL(:,1), ALL(:,3)],[ALL(:,2), ALL(:,4)] );
        ALL_mean = mean(ALL);

        ssim_omp = ssim(im2uint8(X_OMP),im2uint8(Xhat_OMP));
        ssim_eomp = ssim(im2uint8(X_eOMP),im2uint8(Xhat_eOMP));
