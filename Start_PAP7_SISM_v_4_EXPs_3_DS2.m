...
function Start_PAP7_SISM_v_4_EXPs_3_DS2(file_index)
clc;
batchSize = 256;%16,8
sparse = 128;
...
for i = file_index:file_index%322000
    ...
    pre_res  = strcat('PAP7_ALL_VARS_RESULTS/PAP7_OMP_EXP3_DS3');
    dicType_ = 'SDic';
    ...
    save(strcat(pre_res,'_file_','(',int2str(i),')'),'OMP_SDIC_RESULTS_ALL');%,'OMP_RAND_RESULTS_ALL','OMP_MODO_RESULTS_ALL');
    check = 0;
...
for ii = 1:1:length(err_threshold)
    ...
    for jj = 1:1:length(mod_threshold)
        ...
        PARA_RELI_AVG = [sum(PARA_RES(end-255:end,[1 2])),sum(PARA_RES(end-255:end,4)<PARA_RES(end-255:end,3)),sum(PARA_RES(end-255:end,5)<PARA_RES(end-255:end,3)),sum(PARA_RES(end-255:end,6)<PARA_RES(end-255:end,3)),sum(PARA_RES(end-255:end,7)<PARA_RES(end-255:end,3)),...
            sum(PARA_RES(end-255:end,5)<PARA_RES(end-255:end,4)),sum(PARA_RES(end-255:end,6)<PARA_RES(end-255:end,4)), sum(PARA_RES(end-255:end,7)<PARA_RES(end-255:end,4)),...
            sum(PARA_RES(end-255:end,6)<PARA_RES(end-255:end,5)),sum(PARA_RES(end-255:end,7)<PARA_RES(end-255:end,5)),...
            sum(PARA_RES(end-255:end,7)<PARA_RES(end-255:end,6))]./256;
...
Dic = [full(Dic2), Dic1];
save('PAP7_KSVD_LEARNED_256_512','Dic');
...
[Dic2,nbvect] = wmpdictionary(size(Vector,1),'lstcpt',{{'wphaar',2},'dct',});
Dic = [full(Dic2), Dic1];
save('PAP7_MOD_LEARNED_256_512','Dic');
...
[Dic2,nbvect] = wmpdictionary(size(Vector,1),'lstcpt',{{'wphaar',2},'dct',});
Dic = [full(Dic2), Dic1];
save('PAP7_RAND_LEARNED_256_512','Dic');
...
[Dic2,nbvect] = wmpdictionary(size(Vector,1),'lstcpt',{{'wphaar',2},'dct',});
Dic = [full(Dic2), Dic1];
save('PAP7_MODO_LEARNED_256_512','Dic');
check = 0;
end
