...
function Start_PAP7_SISM_v_4_EXPs_1_2_FINAL_FINAL(file_index)
clc;
batchSize = 128;%16,8
sparse = 90;
imDim = 32; %512,vecLength,128
...
for i = file_index:file_index%322000
    ...
    JPEG_RES = [bytes*8/(batchSize*batchSize), jpg_ssim, jpg_psnr, qual];
    [bytes, j2000_ssim, j2000_psnr, c_rat] = getJPEG2000_qual_me_ano(fName);
    JPEG2000_RES = [bytes*8/(batchSize*batchSize), j2000_ssim, j2000_psnr, c_rat];
    pre_res  = strcat('PAP7_ALL_VARS_RESULTS/QQ_PAP7_OMP_EXP12_DS1');
    dicType_ = 'SDic';
    fprintf(strcat(int2str(file_index),' ... (DCT)\n'));%2012

    [OMP_SDIC_RESULTS_ALL] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL','dct');
    dicType_ = 'LDic';
    for m = 1:1:5
        %%%%%%%%%%%%%%%%
        fprintf(strcat(int2str(file_index),' ... (ODL %d:5)\n'),m);%2012
        dic_odl = load(strcat('(',int2str(file_index),')_ODL_128.mat'));
        Dic = dic_odl.Dic{m};
        [OMP_ODL_RESULTS_ALL{m}] = PAP7_main_code_LDIC(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL',2,Dic);
        %%%%%%%%%%%%%%%%
        fprintf(strcat(int2str(file_index),' ... (KSVD %d:5)\n'),m);%2012
        dic_ksvd = load(strcat('(',int2str(file_index),')_KSVD_128.mat'));
        Dic = dic_ksvd.Dic{m};
        [OMP_KSVD_RESULTS_ALL{m}] = PAP7_main_code_LDIC(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL',1,Dic);
        %%%%%%%%%%%%%%%%
        fprintf(strcat(int2str(file_index),' ... (MOD %d:5)\n'),m);%2012
        dic_mod = load(strcat('(',int2str(file_index),')_MOD_128.mat'));
        Dic = dic_mod.Dic{m};
        [OMP_MOD_RESULTS_ALL{m}] = PAP7_main_code_LDIC(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL',2,Dic);
        %%%%%%%%%%%%%%%%
        fprintf(strcat(int2str(file_index),' ... (RBDL %d:5)\n'),m);%2012
        dic_mod = load(strcat('(',int2str(file_index),')_RBDL_128.mat'));
        Dic = dic_mod.Dic{m};
        [OMP_RBDL_RESULTS_ALL{m}] = PAP7_main_code_LDIC(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL',2,Dic);
        %%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%
        fprintf(strcat(int2str(file_index),' ... (BSSDL %d:5)\n'),m);%2012
        dic_mod = load(strcat('(',int2str(file_index),')_BSSDL_128.mat'));
        Dic = dic_mod.Dic{m};
        [OMP_BSSDL_RESULTS_ALL{m}] = PAP7_main_code_LDIC(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL',2,Dic);
        %%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%
        fprintf(strcat(int2str(file_index),' ... (BKSVD %d:5)\n'),m);%2012
        dic_mod = load(strcat('(',int2str(file_index),')_BKSVD_128.mat'));
        Dic = dic_mod.Dic{m};
        [OMP_BKSVD_RESULTS_ALL{m}] = PAP7_main_code_LDIC(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL',2,Dic);
        %%%%%%%%%%%%%%%%
        fprintf(strcat(int2str(file_index),' ... (RKSVD %d:5)\n'),m);%2012
        dic_mod = load(strcat('(',int2str(file_index),')_RKSVD_128.mat'));
        Dic = dic_mod.Dic{m};
        [OMP_RKSVD_RESULTS_ALL{m}] = PAP7_main_code_LDIC(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL',2,Dic);
        %%%%%%%%%%%%%%%%

        
    end
    % [OMP_RAND_RESULTS_ALL] = PAP7_main_code_LDIC(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL','randomized');
    %[OMP_MODO_RESULTS_ALL] = PAP7_main_code_LDIC(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL','modortho');
    %%%%%%%%%%%%%%%%%%%%
    %%% [D, X] = ODL(Y, k, lambda, opts, method)%2016
    %%% [ DictMat , EncoderMat ] = TrainDPL(  TrData, TrLabel, DictSize, %%% tau, lambda, gamma ); %2014


    save(strcat(pre_res,'_file_','(',int2str(i),')'),'OMP_SDIC_RESULTS_ALL','OMP_KSVD_RESULTS_ALL','OMP_MOD_RESULTS_ALL','OMP_ODL_RESULTS_ALL','OMP_RBDL_RESULTS_ALL','OMP_BSSDL_RESULTS_ALL','OMP_BKSVD_RESULTS_ALL','OMP_RKSVD_RESULTS_ALL');%,'OMP_RAND_RESULTS_ALL','OMP_MODO_RESULTS_ALL');
    check = 0;
...
function [RESULTS] = PAP7_main_code(img,ori_bpp,batchSize,sparse_,dicType,JPEG_RES,JPEG2000_RES,IAM_IBM_NOTIFY,LDIC_TYPE) %#ok<*INUSD>
...
Dic = getDic_(dicType,vecLength,LDIC_TYPE);
[VecMatrix,kr,kc] = AAA_getIMGvectors_(img_double,batchSize);
index = 1;
warning('off','all');

err_threshold = 20:-2:2;%35:-5:5;
%mod_threshold = 50:-10:10;
mod_threshold = [64,32,16,8,4];
mod_threshold_ = {[1,64,128],[1,32:32:128],[1,16:16:128],[1,8:8:128],[1,4:4:128]};
OVERALL_TIME = [];
OVERALL_PARA = [];
OVERALL_PARA_RELI = [];
OVERALL_SSIM_avg = [];
...
for ii = 1:1:length(err_threshold)
    ...
    for jj = 1:1:length(mod_threshold)
        ...
        %clc;
        %fprintf(strcat(img,' ... ',dicType,'_',LDIC_TYPE, ' ... ', '(Ours Alg. Reached %.2f%%)\n'),(indicator_-1)*100/(length(err_threshold)*length(mod_threshold)));%2012
        indicator_ = indicator_ + 1;

        for i=1:1:kr
            k = 1;
            for j=1:1:kc
                Vector =  im2double(img_pixels(vecLength*(i-1)+1:i*vecLength,j)); %VecMatrix(i,j,:);
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                if do_one_time == 0
                    for v =1:1:kc
                        Vector_ =  im2double(img_pixels(vecLength*(i-1)+1:i*vecLength,v));
                        tic;
                        ...
                        BPP1(i,v,ii) = (ceil(log2(size(omp_coeff,1))) + bits_per_coeff + (N1(i,v,ii)-1)*(bits_per_coeff + bits_per_index))/vecLength;
                        X_OMP = [X_OMP Vector_];
                ...
                S2 = eomp_SSIM(end,1);
                E2 = eomp_error(end,1);
                BPP2 = (ceil(log2(size(omp_coeff,1))) + bits_per_coeff + (N2-1)*(bits_per_coeff + bits_per_index))/vecLength;
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                S3 = eomp_SSIM(end,1);
                E3 = eomp_error(end,1);
                BPP3 = (ceil(log2(size(omp_coeff,1))) + bits_per_coeff + (N3-1)*(bits_per_coeff + bits_per_index) - 2* bits_per_index)/vecLength;
                alpha_1_3 = alpha_1;
                alpha_2_3 = alpha_2;
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %%%%%%%%%%%%%%%%%%%%%%%%   Prior-Posterior Knowledge based
                ...
                S4 = eomp_SSIM(end,1);
                E4 = eomp_error(end,1);
                BPP4 = (ceil(log2(size(omp_coeff,1))) + bits_per_coeff + (N4-1)*(bits_per_coeff + bits_per_index)-2*bits_per_index)/vecLength;
                alpha_1_4 = alpha_vector(1);
                alpha_2_4 = alpha_vector(2);
                %ANG_S3_S2 = max(min(dot(X_PPKbM_1_init,X_PPKbM_2_init)/(norm(X_PPKbM_1_init)*norm(X_PPKbM_2_init)),1),-1);
                ANG_S3_S2_deg = real(acosd(dot(X_PPKbM_1_init,X_PPKbM_2_init)/(norm(X_PPKbM_1_init)*norm(X_PPKbM_2_init))));
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %%%%%%%%%%%%%%%%%%%%%%%%   Prior-Posterior Knowledge based
                ...
                S5 = eomp_SSIM(end,1);
                E5 = eomp_error(end,1);
                BPP5 = (ceil(log2(size(omp_coeff,1))) + bits_per_coeff + (N5-1)*(bits_per_coeff + bits_per_index))/vecLength;
                ...
                init_SeIB4 = alpha_1_5*Dic_(:,end-1)+alpha_2_5*Dic_(:,end);
                %ANG_S4_S2 = max(min(dot(X_PPKbM_1_init,init_SeIB4)/(norm(X_PPKbM_1_init)*norm(init_SeIB4)),1),-1);
                ANG_S4_S2_deg = real(acosd(dot(X_PPKbM_1_init,init_SeIB4)/(norm(X_PPKbM_1_init)*norm(init_SeIB4))));
                ...
                MSE_RES = [MSE_RES;err_threshold(ii), mod_threshold(jj) E1(i,j,ii) E2 E3 E4 E5];
                ...
                if alpha_1_3 + alpha_2_3 + alpha_1_4 + alpha_2_4 + alpha_1_5 + alpha_2_5 == 0
                    ...
                    theta_degree = [theta_degree;err_threshold(ii), mod_threshold(jj), ANG_S3_S2_deg, ANG_S4_S2_deg];
                end
        ...
        MSE_OMP = [immse(Xhat_OMP,X_OMP), immse(OMP_ALG1,X_OMP),immse(OMP_ALG2,X_OMP)];
        MSE_PKbM = [immse(X_PKbM,X_OMP), immse(PKbM_ALG1,X_OMP),immse(PKbM_ALG2,X_OMP)];
        MSE_PPKbM_1 = [immse(X_PPKbM_1,X_OMP), immse(PPKbM_1_ALG1,X_OMP),immse(PPKbM_1_ALG2,X_OMP)];
        MSE_PPKbM_2 = [immse(X_PPKbM_2,X_OMP), immse(PPKbM_2_ALG1,X_OMP),immse(PPKbM_2_ALG2,X_OMP)];
        MSE_UDbM = [immse(X_UDbM,X_OMP), immse(UDbM_ALG1,X_OMP),immse(UDbM_ALG2,X_OMP)];

        
        NMSE_OMP = [imnmse(Xhat_OMP,X_OMP), imnmse(OMP_ALG1,X_OMP),imnmse(OMP_ALG2,X_OMP)];
        NMSE_PKbM = [imnmse(X_PKbM,X_OMP), imnmse(PKbM_ALG1,X_OMP),imnmse(PKbM_ALG2,X_OMP)];
        NMSE_PPKbM_1 = [imnmse(X_PPKbM_1,X_OMP), imnmse(PPKbM_1_ALG1,X_OMP),imnmse(PPKbM_1_ALG2,X_OMP)];
        NMSE_PPKbM_2 = [imnmse(X_PPKbM_2,X_OMP), imnmse(PPKbM_2_ALG1,X_OMP),imnmse(PPKbM_2_ALG2,X_OMP)];
        NMSE_UDbM = [imnmse(X_UDbM,X_OMP), imnmse(UDbM_ALG1,X_OMP),imnmse(UDbM_ALG2,X_OMP)];
        ...
        [~, ~, Q_OMP_COEFF_bpp ] = get_prob_and_entropy(Q_OMP_COEFF_ind,vecLength);
        [~, ~, Q_PKbM_COEFF_bpp ] = get_prob_and_entropy(Q_PKbM_COEFF_ind,vecLength);
        [~, ~, Q_PPKbM_1_COEFF_bpp ] = get_prob_and_entropy(Q_PPKbM_1_COEFF_ind,vecLength);
        [~, ~, Q_PPKbM_2_COEFF_bpp ] = get_prob_and_entropy(Q_PPKbM_2_COEFF_ind,vecLength);
        [~, ~, Q_UDbM_COEFF_bpp ] = get_prob_and_entropy(Q_UDbM_COEFF_ind,vecLength);

        [~, ~, OMP_IND_bpp ] = get_prob_and_entropy(OMP_IND,vecLength);
        [~, ~, PKbM_IND_bpp ] = get_prob_and_entropy(PKbM_IND,vecLength);
        [~, ~, PPKbM_1_IND_bpp ] = get_prob_and_entropy(PPKbM_1_IND,vecLength);
        [~, ~, PPKbM_2_IND_bpp ] = get_prob_and_entropy(PPKbM_2_IND,vecLength);
        [~, ~, UDbM_IND_bpp ] = get_prob_and_entropy(UDbM_IND,vecLength);



        BPP_AVG = sum(BPP_RES(end-vecLength+1:end,:),1)/vecLength;
        ...
        TIME_AVG = sum(TIME_RES(end-vecLength+1:end,:),1)/vecLength;
        PARA_AVG = sum(PARA_RES(end-vecLength+1:end,:),1)/vecLength;
        PARA_RELI_AVG = [sum(PARA_RES(end-vecLength+1:end,[1 2])),sum(PARA_RES(end-vecLength+1:end,4)<PARA_RES(end-vecLength+1:end,3)),sum(PARA_RES(end-vecLength+1:end,5)<PARA_RES(end-vecLength+1:end,3)),sum(PARA_RES(end-vecLength+1:end,6)<PARA_RES(end-vecLength+1:end,3)),sum(PARA_RES(end-vecLength+1:end,7)<PARA_RES(end-vecLength+1:end,3)),...
            sum(PARA_RES(end-vecLength+1:end,5)<PARA_RES(end-vecLength+1:end,4)),sum(PARA_RES(end-vecLength+1:end,6)<PARA_RES(end-vecLength+1:end,4)), sum(PARA_RES(end-vecLength+1:end,7)<PARA_RES(end-vecLength+1:end,4)),...
            sum(PARA_RES(end-vecLength+1:end,6)<PARA_RES(end-vecLength+1:end,5)),sum(PARA_RES(end-vecLength+1:end,7)<PARA_RES(end-vecLength+1:end,5)),...
            sum(PARA_RES(end-vecLength+1:end,7)<PARA_RES(end-vecLength+1:end,6))]./vecLength;
        SSIM_AVG = sum(SSIM_RES(end-vecLength+1:end,:),1)/vecLength;
        MSE_AVG = sum(MSE_RES(end-vecLength+1:end,:),1)/vecLength;
        ALPHAS_AVG = sum(Alphas(1:end,:),1)/size(Alphas,1);
        theta_degree_AVG = sum(theta_degree(1:end,:),1)/size(theta_degree,1);

        OVERALL_TIME = [OVERALL_TIME; TIME_AVG];
        OVERALL_PARA = [OVERALL_PARA; PARA_AVG];
        OVERALL_PARA_RELI = [OVERALL_PARA_RELI;PARA_RELI_AVG];
        ...
        OVERALL_MSE_avg = [OVERALL_MSE_avg; MSE_AVG];
        OVERALL_MSE_FIN = [OVERALL_MSE_FIN; MSE_OMP MSE_PKbM MSE_PPKbM_1 MSE_PPKbM_2 MSE_UDbM];
        ...
        OVERALL_DEGREES = [OVERALL_DEGREES; theta_degree_AVG];
        Alphas = [];
        theta_degree =[];
        %PARA_RELI_AVG = [];
...
RESULTS.OVERALL_TIME = flip(OVERALL_TIME,1);
RESULTS.OVERALL_PARA = flip(OVERALL_PARA,1);
RESULTS.OVERALL_PARA_RELI = flip(OVERALL_PARA_RELI,1);
RESULTS.OVERALL_SSIM_avg = flip(OVERALL_SSIM_avg,1);
RESULTS.OVERALL_SSIM_FIN = flip(OVERALL_SSIM_FIN,1);
RESULTS.OVERALL_MSE_avg = flip(OVERALL_MSE_avg,1);
RESULTS.OVERALL_NMSE_FIN = flip(OVERALL_NMSE_FIN,1);
RESULTS.OVERALL_PSNR_FIN = flip(OVERALL_PSNR_FIN,1);
RESULTS.OVERALL_BPP  = flip(OVERALL_BPP,1);
RESULTS.OVERALL_ALPHAS = flip(OVERALL_ALPHAS,1);
RESULTS.OVERALL_DEGREES = flip(OVERALL_DEGREES,1);
...
function [RESULTS] = PAP7_main_code_LDIC(img,ori_bpp,batchSize,sparse_,dicType,JPEG_RES,JPEG2000_RES,IAM_IBM_NOTIFY,LDIC_TYPE,Aux_Dic) %#ok<*INUSD>
%BRE_IMG Summary of this function goes here
%   Detailed explanation goes here
global bits_per_coeff;
global bits_per_index;
...
vecLength = batchSize;%batchSize*batchSize;
img_double = im2double(img_pixels);
%dicType,vecLength,LDIC_TYPE,img_double
%
[VecMatrix,kr,kc] = AAA_getIMGvectors_(img_double,batchSize);
index = 1;
warning('off','all');
...
indicator_ = 1;
Dic_ = getDic_new(dicType,vecLength,LDIC_TYPE,img_double);
Dic = [Aux_Dic Dic_];
%omp_ci_matrix = zeros(size(Dic,2),kr*kc,length(err_threshold));
%eomp_ci_matrix = zeros(size(Dic,2),kr*kc,length(err_threshold),length(mod_threshold));
%No_ATOMS{1} = ceil(size(img_double,2)/mod_threshold(1));
%No_ATOMS{2} = ceil(size(img_double,2)/mod_threshold(2));

%Dic_{1} = getDic_new(dicType,vecLength,LDIC_TYPE,img_double,No_ATOMS{1});
%Dic_{2} = getDic_new(dicType,vecLength,LDIC_TYPE,img_double,No_ATOMS{2});
...
for ii = 1:1:length(err_threshold)
    ...
    for jj = 1:1:length(mod_threshold)
        %No_ATOMS = ceil(size(img_double,2)/mod_threshold(jj));
        %%Dic = Dic_{jj};
        %DicCoh = Dic'*Dic;
        notify = 0;
        if do_one_time == 0
        ...
        SSIM_AVG = sum(SSIM_RES(end-vecLength+1:end,:),1)/vecLength;
        MSE_AVG = sum(MSE_RES(end-vecLength+1:end,:),1)/vecLength;
        %NMSE_AVG = sum(NMSE_RES(end-vecLength+1:end,:),1)/vecLength;
        ALPHAS_AVG = sum(Alphas(1:end,:),1)/size(Alphas,1);
        ...
        OVERALL_SSIM_FIN = [OVERALL_SSIM_FIN; SSIM_OMP];% SSIM_PKbM SSIM_PPKbM_1 SSIM_PPKbM_2 SSIM_UDbM];
        OVERALL_MSE_avg = [OVERALL_MSE_avg; MSE_AVG];
        OVERALL_MSE_FIN = [OVERALL_MSE_FIN; MSE_OMP];% NMSE_PKbM NMSE_PPKbM_1 NMSE_PPKbM_2 NMSE_UDbM];
        OVERALL_NMSE_FIN = [OVERALL_NMSE_FIN; NMSE_OMP];% NMSE_PKbM NMSE_PPKbM_1 NMSE_PPKbM_2 NMSE_UDbM];
        OVERALL_PSNR_FIN = [OVERALL_PSNR_FIN; PSNR_OMP];% PSNR_PKbM PSNR_PPKbM_1 PSNR_PPKbM_2 PSNR_UDbM];
        OVERALL_BPP  = [OVERALL_BPP; BPP_AVG, BPP_ENT_AVG];
        %OVERALL_ALPHAS = [OVERALL_ALPHAS; ALPHAS_AVG];
...
RESULTS.OVERALL_SSIM_FIN = flip(OVERALL_SSIM_FIN,1);
RESULTS.OVERALL_MSE_avg = flip(OVERALL_MSE_avg,1);
RESULTS.OVERALL_MSE_FIN = flip(OVERALL_MSE_FIN,1);
...
function       [entropy_, total_bits, bpp_entropy ] = get_prob_and_entropy(sequence,vecLength)
sequence_ = sequence;
output_ = [];
...
bpp_entropy = total_bits /(vecLength*vecLength);
check = 0;
end
...
while curr_level < stop && err_>err_threshold %local_ssim_old(end,1)<= ssim_threshold
    ...
    temp = y - y_hat;
    %%%%%%%%%
    curERR(count,1) = norm(temp)^2;
    err_ = curERR(count,1)*100/normy^2;
...
function [Dic] = getDic_(dicType,vecLength,LDIC_TYPE)
if dicType == 'LDic'
    %Dic = getLearnedAtoms_();
    switch LDIC_TYPE
        case 'ksvd'
            dd = load('PAP7_KSVD_LEARNED_vecLength_512.mat');
        case 'mod'
            dd = load('PAP7_MOD_LEARNED_vecLength_512.mat');
        case 'randomized'
            dd = load('PAP7_RAND_LEARNED_vecLength_512.mat');
        case 'modortho'
            dd = load('PAP7_MODO_LEARNED_vecLength_512.mat');
    end
    %dd = load('MOD_LEARNED_vecLength_512.mat');
    Dic = dd.Dic;
else

    [Dic_r,nbvect] = wmpdictionary(vecLength,'lstcpt',{'dct',});%{'wphaar',2},,{'wpsym4',2}}'dct'
    aa = ones(vecLength,1);
...
k = 1;
for i = 1:1:10
    img = imread(strcat('(',int2str(file(i)),').jpg'));
    img_double = im2double(img);
    imgDouble(:,:)=img_double(:,:,1);
    [Batches, batchPerRow,batchPerCol] = getBatches_(imgDouble,16);
    for ii=1:1:batchPerRow
        for jj=1:1:batchPerCol
            Vector(:,k) = batchToVector_(Batches(ii,jj,:,:),16);
            k = k + 1;
        end
    end
end
...
Dic = [full(Dic2), Dic1];
save('PAP7_KSVD_LEARNED_vecLength_512','Dic');
...
[Dic2,nbvect] = wmpdictionary(size(Vector,1),'lstcpt',{{'wphaar',2},'dct',});
Dic = [full(Dic2), Dic1];
save('PAP7_MOD_LEARNED_vecLength_512','Dic');
...
[Dic2,nbvect] = wmpdictionary(size(Vector,1),'lstcpt',{{'wphaar',2},'dct',});
Dic = [full(Dic2), Dic1];
save('PAP7_RAND_LEARNED_vecLength_512','Dic');
...
[Dic2,nbvect] = wmpdictionary(size(Vector,1),'lstcpt',{{'wphaar',2},'dct',});
Dic = [full(Dic2), Dic1];
save('PAP7_MODO_LEARNED_vecLength_512','Dic');
check = 0;
end


function [Dic] = getDic_new(dicType,vecLength,LDIC_TYPE,img_double,No_ATOMS)
...
if DIC_LEAR_ALG == 1
    options.learning_method = 'ksvd';
    options.niter_learning = 80;
    options.K = No_ATOMS;
    options.nbr_max_atoms = 60;
    [Dic1,X,E] = perform_dictionary_learning(Vector, options);
    [Dic2,nbvect] = wmpdictionary(size(Vector,1),'lstcpt',{{'wphaar',2},'dct',});
    Dic = [full(Dic2), Dic1];
    %save('PAP7_KSVD_LEARNED_vecLength_512','Dic');
