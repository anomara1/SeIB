function [bytes, jpg_ssim, jpg_psnr, qual, allIMGs] = getJPEG_qualme( img_file, jpg_qual )
  ...
  for i=1:n
    ...
    Ijpg_(:,:,i) = im2double(Ijpg(:,:,1));
    
    jpg_ssim(i) = ssim(Iorig, Ijpg_(:,:,i));
    sav_ssim(i) = jpg_ssim(i);
    
    
    jpg_psnr(i) = psnr(Iorig, Ijpg_(:,:,i));
