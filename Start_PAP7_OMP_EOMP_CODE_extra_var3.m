...
function Start_PAP7_OMP_EOMP_CODE_extra_var3(file_index)
clc;
batchSize = 256;%16,8
sparse = 128;
...
for i = file_index:file_index%322000
    ...
    pre_res  = strcat('PAP7_OMP_EOMP_VAR3_RESULTS/PAP7_OMP_EOMP_VAR3');
    dicType_ = 'SDic';
    %[STRUCTURED_RESULTS_SA] = AAA_Second_BE_Final_SparseALGs_enh_PAP5(fName,batchSize,sparse,dicType_);
    [SDIC_RESULTS_VAR3] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES);
    dicType_ = 'LDic';
    fName = strcat('(',int2str(i),')',imExt);
    %[LEARNED_RESULTS_SA] = AAA_Second_BE_Final_SparseALGs_enh_PAP5(fName,batchSize,sparse,dicType_);
    LDIC_RESULTS_VAR3 = [];
    %[LDIC_RESULTS_VAR3] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES);
    
    
    save(strcat(pre_res,'_file_','(',int2str(i),')'),'SDIC_RESULTS_VAR3','LDIC_RESULTS_VAR3');
    check = 0;
    ...
    for jj = 1:1:length(mod_threshold)
        ...
        for i=1:1:kr
            ...
            for j=1:1:kc
                ...
                S2 = eomp_SSIM(end,1);
                E2 = eomp_error(end,1);
                BPP2 = (ceil(log2(size(omp_coeff,1))) + bits_per_coeff + (N2-1)*(bits_per_coeff + bits_per_index)+2*bits_per_coeff)/256;
       ...
       % AA = ssim_(Xhat_OMP_),im2uint8(X_OMP_));
        ...
        [~,QEOMP_COEFF_ind] = imquantize(EOMP_COEFF(~isnan(EOMP_COEFF))',-5:0.0196:5,0:1:511);
...
for j = 1:1:size(y_hat,2)
    for jj = 1:1:length(fix_ind)-1
        if j>fix_ind(jj) && j<fix_ind(jj+1)
            i1 = fix_ind(jj);
            i2 = fix_ind(jj+1);
            y_i1 = y_hat(:,i1)/norm(y_hat(:,i1),2);
            y_i2 = y_hat(:,i2)/norm(y_hat(:,i2),2);
            alpha_vector = inv([y_i1, y_i2]'*[y_i1, y_i2])*[y_i1, y_i2]'* y(:,j) ;%init_mat(:,j) = 1 * ((j-i1)/(i2-i1-1) * y_hat(:,i2) + (1-(j-i1)/(i2-i1-1)) * y_hat(:,i1));
            
            init_mat(:,j)=[y_i1, y_i2]*alpha_vector;
