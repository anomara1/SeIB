...
function Start_PAP7_ALGs_EALGs_CODE_extra(file_index)
clc;
batchSize = 256;%16,8
sparse = 128;
...
try
    for i = file_index:file_index%322000
        ...
        pre_res  = strcat('PAP7_RESULTS_DS3/PAP7_FBP_EFBP');
        dicType_ = 'SDic';
    ...
    for jj = 1:1:length(mod_threshold)
        ...
        for i=1:1:kr
            ...
            for j=1:1:kc
                ...
                if do_one_time == 0
                    tic;
                    [~,omp_hat,omp_coeff,omp_iopt,omp_error,omp_ssim] = FBP_PAP7(Vector, Dic, 2, 1, 'K', 0, sparse,err_threshold(ii));

                    alpha = 0.2;
                    [COEFFS_OMPvar,OMPvar_ERROR,OMPvar_SSIM]    = OMPvar_pap7(Dic,Vector,sparse,alpha,0,err_threshold(ii));%2015
                    [COEFFS_CSMPSP,CSMPSP_SSIM,CSMPSP_ERROR]    = CSMPSP_pap7(Vector,Dic,sparse,{0 5},err_threshold(ii));%2015
                    [COEFFS_gOMP,gOMP_SSIM,gOMP_ERROR]          = gOMP_pap7(Vector,Dic,sparse,{0 5 4},err_threshold(ii));%2012
                    [COEFFS_ROMP,ROMP_SSIM,ROMP_ERROR]          = ROMP_pap7(Vector,Dic,sparse,{0 5},err_threshold(ii));%2009
                    [COEFFS_StOMP,StOMP_SSIM,StOMP_ERROR]       = StOMP_pap7(Vector,Dic,sparse,{0 5 3},err_threshold(ii));%2012
                    [COEFFS_NIHT,NIHT_SSIM,NIHT_ERROR]          = NIHT_pap7(Vector,Dic,sparse,{0 20},err_threshold(ii));%2010
                    [COEFFS_hard_l0, err_mse, iter_time,HL0T_SSIM,HL0T_ERROR]  = hard_l0_Mterm_pap7(Vector,Dic,size(Dic,2),sparse,err_threshold(ii));
                    [COEFFS_iht, r_mag,IHT_SSIM,IHT_ERROR]      = iht_pap7(Vector, Dic, 50, 0,err_threshold(ii));
                ...
                [~,eomp_hat,eomp_coeff,eomp_iopt,eomp_error,eomp_SSIM] = Enh_FBP_PAP7(Vector, Dic, 2, 1, 'K', 0, sparse,err_threshold(ii),y_init);
...
function [y_FBP,FBP_hat,Coefs,T,EFBP,fbp_ssim] = FBP_PAP7(y, V, alpha, beta, TerMode, eps, K,THRESHOLD_L2)
%~,E_FBP,C_FBP,I_FBP,EFBP,time_fbp

%D1(:,:,i), Di1, beta1(:,:,i), C_OOMP(:,i),I_OOMP(:,i),E_OOMP(:,:,i), P_OOMP(:,i),M_OOMP(:,i)
...
%
% Burak Karahano̡lu, Sabanci University, 2012
...
%tic;
while norm(Res)*100/norm(y) > bound % for termination mode 'Err'
    ...
    y_FBP = V(:,T)*Coefs;
    Res = y - y_FBP;
    y_i_FBP(:,length(T)) = y_FBP;
    %SSIM_FBP(length(T)) = ssim(y,y_FBP);
    %curERR(length(T)) = immse(y,y_FBP);

    %[PesqMos] = comp_pesq_GLOBE(y,16000,y_FBP,16000);
    %PESQMat(length(T),1) = 0;%PesqMos(1);
    %MOSMat(length(T),1) = 0;%PesqMos(2);

    Err_Mat(:,length(T)) = y - y_FBP;
    FBP_hat(:,length(T)) = y_FBP;
    EFBP(length(T),1) = immse(y, y_FBP);%Err_(k,1) = immse(f,app);
...
x = zeros(N,1);
x(T) = Coefs(myInd);
fbp_ssim = ssim_(y, y_FBP);
check = 0;
end



function [y_FBP,FBP_hat,Coefs,T,EFBP,fbp_ssim] = Enh_FBP_PAP7(y, V, alpha, beta, TerMode, eps, K,THRESHOLD_L2,y_init)
%~,E_FBP,C_FBP,I_FBP,EFBP,time_fbp
...
while norm(Res)*100/norm(y) > bound % for termination mode 'Err'
    ...
    % backward step:
    Coefs = V(:,T)\(y-y_init); % orthogonal projection
    [a, ind] = sort(abs(Coefs));
    T(ind(1:beta)) = []; % remove the worst beta from support
    ...
    Err_Mat(:,length(T)) = y - y_FBP-y_init;
    FBP_hat(:,length(T)) = y_FBP+y_init;
    EFBP(length(T),1) = immse(y, y_FBP+y_init);%Err_(k,1) = immse(f,app);
    if length(T)>=noMaxSupDim % for termination mode 'K'
        break;
    end
end
