...
function Start_PAP7_SISM_v_4_EXPs_3_DS2_For_Delay_Only(file_index)
clc;
batchSize = 128;%vecLength;%16,8
sparse = 90;
...
for i = file_index:file_index%322000
    ...
    pre_res  = strcat('PAP7_ALL_VARS_RESULTS/AAAAA_DELAY_ENV_3_LAPTOP_ERI');

     dicType_ = 'SDic';
    fprintf(strcat(int2str(file_index),' ... (DCT) (1:8)\n'));%2012
    [OMP_SDIC_RESULTS_ALL] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL','dct');
   
    
    dicType_ = 'LDic';
    fprintf(strcat(int2str(file_index),' ... (BSSDL) (2:8)\n'));%2012
    [OMP_BSSDL_RESULTS_ALL] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL','bssdl');
    
    dicType_ = 'LDic';
    fprintf(strcat(int2str(file_index),' ... (RBDL) (3:8)\n'));%2012
    [OMP_RBDL_RESULTS_ALL] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL','rbdl');

    dicType_ = 'LDic';
    fprintf(strcat(int2str(file_index),' ... (BKSVD) (4:8)\n'));%2012
    [OMP_BKSVD_RESULTS_ALL] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL','bksvd');

    
    
    dicType_ = 'LDic';
    fprintf(strcat(int2str(file_index),' ... (KSVD) (5:8)\n'));%2012
    [OMP_KSVD_RESULTS_ALL] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL','ksvd');
    
    dicType_ = 'LDic';
    fprintf(strcat(int2str(file_index),' ... (MOD) (6:8)\n'));%2012
    [OMP_MOD_RESULTS_ALL] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL','mod');
    
    dicType_ = 'LDic';
    fprintf(strcat(int2str(file_index),' ... (ODL) (7:8)\n'));%2012
    [OMP_ODL_RESULTS_ALL] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL','odl');
    
    dicType_ = 'LDic';
    fprintf(strcat(int2str(file_index),' ... (RKSVD) (8:8)\n'));%2012
    [OMP_RKSVD_RESULTS_ALL] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ALL','rksvd');
...
function [RESULTS] = PAP7_main_code_xx(img,ori_bpp,batchSize,sparse_,dicType,JPEG_RES,JPEG2000_RES,IAM_IBM_NOTIFY,LDIC_TYPE) %#ok<*INUSD>
%BRE_IMG Summary of this function goes here
%   Detailed explanation goes here
global bits_per_coeff;
global bits_per_index;
    ...
    for jj = 1:1:length(mod_threshold)
        ...
        for i=1:1:kr
            ...
            for j=1:1:kc
                ...
                [~, eomp_coeff, eomp_iopt, eomp_error,eomp_SSIM, eomp_hat]       = Enh_Convergence_OMP_pap7_(Dic_, Vector,sparse,X_UDbM_init,1, err_threshold(ii));
                ind_to_rem = find(eomp_coeff == 0);
                eomp_coeff(ind_to_rem) = [];
                eomp_iopt(ind_to_rem) = [];
                eomp_error(ind_to_rem) = [];
                eomp_hat(:,ind_to_rem) = [];
                UDbM_COEFF = [UDbM_COEFF;eomp_coeff];
                UDbM_IND = [UDbM_IND;eomp_iopt];
                X_UDbM(:,j) = eomp_hat(:,end);
...
RESULTS.OVERALL_SSIM_avg = flip(OVERALL_SSIM_avg,1);
RESULTS.OVERALL_SSIM_FIN = flip(OVERALL_SSIM_FIN,1);
RESULTS.OVERALL_NMSE_avg = flip(OVERALL_NMSE_avg,1);
RESULTS.OVERALL_NMSE_FIN = flip(OVERALL_NMSE_FIN,1);
RESULTS.OVERALL_BPP  = flip(OVERALL_BPP,1);
RESULTS.OVERALL_ALPHAS = flip(OVERALL_ALPHAS,1);
RESULTS.OVERALL_DEGREES = flip(OVERALL_DEGREES,1);
...
if dicType == 'LDic'
    ...
    switch LDIC_TYPE
        case 'ksvd'
            dd = load('EXP4_KSVD_DIC_128.mat');
        case 'mod'
            dd = load('EXP4_MOD_DIC_128.mat');
        case 'odl'
            dd = load('EXP4_ODL_DIC_128.mat');
        case 'bksvd'
            dd = load('EXP4_BKSVD_DIC_128.mat');
        case 'bssdl'
            dd = load('EXP4_BSSDL_DIC_128.mat');
        case 'rbdl'
            dd = load('EXP4_RBDL_DIC_128.mat');

        case 'rksvd'
            dd = load('EXP4_RKSVD_DIC_128.mat');
    end
