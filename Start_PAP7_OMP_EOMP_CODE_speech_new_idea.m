...
function Start_PAP7_OMP_EOMP_CODE_speech_new_idea(file_index)
clc;
batchSize = 256;%16,8
sparse = 128;
...
for i = file_index:file_index%322000
    ...
    %else
        ori_bps = 0;
    %end
    fName = strcat('(',int2str(i),')_8khz',imExt);
    pre_res  = strcat('PAP7_RESULTS_DS3_speech/PAP7_OMP_EOMP_SPEECH');
    dicType_ = 'SDic';
    %[STRUCTURED_RESULTS_SA] = AAA_Second_BE_Final_SparseALGs_enh_PAP5(fName,batchSize,sparse,dicType_);
    [SDIC_RESULTS] = PAP7_main_code(fName,ori_bps,batchSize,sparse,dicType_,file_index);
    dicType_ = 'LDic';
    fName = strcat('(',int2str(i),')',imExt);
    %[LEARNED_RESULTS_SA] = AAA_Second_BE_Final_SparseALGs_enh_PAP5(fName,batchSize,sparse,dicType_);
    [LDIC_RESULTS] = PAP7_main_code(fName,ori_bps,batchSize,sparse,dicType_,file_index);
...
function [RESULTS] = PAP7_main_code(img,ori_bpp,batchSize,sparse_,dicType,file_index) %#ok<*INUSD>
...
global sparse;
sparse = sparse_;
img_pixels = (audioread(img))';
iter = 0;
vecLength = batchSize;%batchSize*batchSize;
global width_ ;
width_ = floor(length(img_pixels)/vecLength);
img_double = prepare_sp_vec(img_pixels,vecLength);
Dic = getDic_(dicType,vecLength);
[kr,kc] = AAA_getIMGvectors_(img_double,batchSize);
index = 1;
warning('off','all');

ssim_threshold = 0.4:0.1:0.9;
err_threshold = 20:5:50;
mod_threshold = 8:8:64;%2:2:30;
OVERALL_RESULTS = [];
...
for ii = 1:1:length(err_threshold)
    ...
    for jj = 1:1:length(mod_threshold)
        ...
        for i=1:1:kr
            k = 1;
            for j=1:1:kc
                Vector =  img_double(:,j);%img_double(:,i);%im2double(img_pixels(256*(i-1)+1:i*256,j)); %VecMatrix(i,j,:);
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                ...
                if do_one_time == 0
                    ...
                    if size(omp_iopt,1) == 1
                        SIZE_2 = (bits_per_NoCoeff + bits_per_coeff + (size(omp_iopt,1)-1)*(bits_per_coeff + bits_per_index));
                        Approx_ = omp_hat(:,end);
                    else
                    ...
                    BPP1(i,j,ii) = (bits_per_NoCoeff + bits_per_coeff + (N1(i,j,ii)-1)*(bits_per_coeff + bits_per_index))/256;
                ...
                S2 = eomp_SSIM(end,1);
                E2 = eomp_error(end,1);
                BPP2 = (bits_per_NoCoeff + bits_per_coeff + (N2-1)*(bits_per_coeff + bits_per_index))/256;
        ...
        ORI = img_pixels(1,1:width_*vecLength);
        %X_OMP = 
        X_OMP = to_vec(Xhat_OMP_);
        err_1 = ORI - X_OMP;
        X_OMP_BRe = to_vec(Xhat_OMP_BRe_);
        X_EOMP = to_vec(Xhat_eOMP_);
        X_EOMP_BRe = to_vec(Xhat_eOMP_BRe_);
        RES_OMP = getCompResults_pap7(ORI,X_OMP,file_index);
        RES_OMP_BRe = getCompResults_pap7(ORI,X_OMP_BRe, file_index);

        RES_EOMP = getCompResults_pap7(ORI,X_EOMP, file_index);
        RES_EOMP_BRe = getCompResults_pap7(ORI,X_EOMP_BRe, file_index);
        check = 0 ;
        ...
        %SSIM_OMP = [ssim(Xhat_OMP_,X_OMP_), ssim(OMP_ALG1,X_OMP_),ssim(OMP_ALG2,X_OMP_)];
        %SSIM_OMP_BRe = [ssim(Xhat_OMP_BRe_,X_OMP_), ssim(OMP_BRe_ALG1,X_OMP_),ssim(OMP_BRe_ALG2,X_OMP_)];
        %SSIM_EOMP = [ssim(Xhat_eOMP_,X_eOMP_), ssim(EOMP_ALG1,X_eOMP_),ssim(EOMP_ALG2,X_eOMP_)];
        %SSIM_EOMP_BRe = [ssim(Xhat_eOMP_BRe_,X_OMP_), ssim(EOMP_BRe_ALG1,X_OMP_),ssim(EOMP_BRe_ALG2,X_eOMP_)];
...
%pesq_mos()
        ...
        BPP_OMP = sum(ALL_STHRE_MTHRE_NSEBT(end-width_+1:end,7))/256;
        BPP_OMP_BRe = sum(ALL_STHRE_MTHRE_NSEBT(end-width_+1:end,8))/256- bits_per_index/256;
        BPP_OMP_ent = QOMP_COEFF_bpp + OMP_IND_bpp;
        BPP_OMP_BRe_ent = BPP_OMP_BRe * (BPP_OMP/BPP_OMP_ent)^(-1);

        %BPP_ORI = ALL_STHRE_MTHRE_NSEBT(end,6);
        BPP_EOMP = sum(ALL_STHRE_MTHRE_NSEBT(end-width_+1:end,14))/256;
        BPP_EOMP_BRe = sum(ALL_STHRE_MTHRE_NSEBT(end-width_+1:end,15))/256 - bits_per_index/256;% The index of the DC components is subtracted
...
function [kr,kc ] = AAA_getIMGvectors_(IMG,batchWidth)
...
function X = prepare_sp_vec(Y,width)
X = [];
z = floor(length(Y)/width);
for i = 1:1:width
    X = [X; Y(1,(i-1)*z+1:i*z)];
end
%for i = 1:1:z
 %   X = [X; Y(1,(i-1)*width+1:i*width)];
%end
end

function [RESULTS] = getCompResults_pap7(XXX,YYY, file_index)

%UNTITLED Summary of this function goes here
%   Detailed explanation goes here
cleanFile = strcat(int2str(file_index),'_cleanTest.wav');
corruFile = strcat(int2str(file_index),'_corruTest.wav');
audiowrite(cleanFile,XXX,8000);
audiowrite(corruFile,YYY,8000);
RESULTS.NMSE = norm(YYY-XXX)/norm(XXX);
RESULTS.SNR = snr(XXX,YYY);
RESULTS.snr_ = comp_snr(cleanFile, corruFile);
%RESULTS.SIZE_KBYTES = SIZE/(8*1024);
%RESULTS.SIZE_bpsample = SIZE/number_of_samples;



%RESULTS.number_of_segments = number_of_segments;
%RESULTS.number_of_samples = number_of_samples;
%RESULTS.iterations = iterations;
%RESULTS.NoCombs = NoCombs;
%RESULTS.Total_time = time;
%RESULTS.TimePerSeg = time/number_of_segments;
%RESULTS.TimePerIte = time/iterations;

%RESULTS.NoCombs = NoCombs;

RESULTS.wss_dist_ = comp_wss(cleanFile, corruFile);
RESULTS.pesq_= comp_pesq(cleanFile, corruFile);
RESULTS.cep_= comp_cep(cleanFile, corruFile);
RESULTS.fwseg_= comp_fwseg(cleanFile, corruFile);
RESULTS.is_= comp_is(cleanFile, corruFile);
RESULTS.stoi_ = stoi(XXX, full(YYY), 8000);
RESULTS.estoi_ = estoi(XXX, full(YYY), 8000);

[RESULTS.Csig_,RESULTS.Cbak_,RESULTS.Covl_]= composite(cleanFile, corruFile);
end

function VEC = to_vec(MAT)
VEC = [];
