...
function Start_PAP7_OMP_EOMP_CODE_extra_var1(file_index)
clc;
batchSize = 256;%16,8
sparse = 128;
...
for i = file_index:file_index%322000
    ...
    pre_res  = strcat('PAP7_OMP_EOMP_VAR1_RESULTS/PAP7_OMP_EOMP_VAR1');
    dicType_ = 'SDic';
    %[STRUCTURED_RESULTS_SA] = AAA_Second_BE_Final_SparseALGs_enh_PAP5(fName,batchSize,sparse,dicType_);
    [SDIC_RESULTS_VAR1] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES);
    dicType_ = 'LDic';
    fName = strcat('(',int2str(i),')',imExt);
    %[LEARNED_RESULTS_SA] = AAA_Second_BE_Final_SparseALGs_enh_PAP5(fName,batchSize,sparse,dicType_);
    LDIC_RESULTS_VAR1 = [];
    %[LDIC_RESULTS_VAR1] = PAP7_main_code(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES);
    
    
    save(strcat(pre_res,'_file_','(',int2str(i),')'),'SDIC_RESULTS_VAR1','LDIC_RESULTS_VAR1');
    check = 0;
