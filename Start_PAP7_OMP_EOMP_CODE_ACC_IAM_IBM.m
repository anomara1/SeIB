...
function Start_PAP7_OMP_EOMP_CODE_ACC_IAM_IBM(file_index)
clc;
batchSize = 256;%16,8
sparse = 128;
...
for i = file_index:file_index%322000
    ...
    pre_res  = strcat('PAP7_OMP_EOMP_ACC_IAM_IBM_RESULTS/PAP7_OMP_EOMP_ACC_IAM_IBM');
    dicType_ = 'SDic';
    [SDIC_RESULTS_ACC] = PAP7_main_code_ACC(fName,ori_bpp,batchSize,sparse,dicType_,JPEG_RES,JPEG2000_RES,'ACC');
    ...
    save(strcat(pre_res,'_file_','(',int2str(i),')'),'SDIC_RESULTS_ACC','SDIC_RESULTS_IAM','SDIC_RESULTS_IBM');
    check = 0;
    
end

end
function [RESULTS] = PAP7_main_code_ACC(img,ori_bpp,batchSize,sparse_,dicType,JPEG_RES,JPEG2000_RES,Method_Notify) %#ok<*INUSD>
    ...
    for jj = 1:1:length(mod_threshold)
        ...
        for i=1:1:kr
            ...
            for j=1:1:kc
                ...
                if IAM_IBM_NOTIFY == 'IAM'
                    if update_init_matrix == 0
                        [init_matrix, alpha_vector(:,jj)] = prep_init_matrix(X_OMP,Xhat_OMP,mod_threshold(jj));
                    ...
                    do_one_time = 1;
                    y_init = zeros(size(X_OMP,1),1);%init_matrix(:,j);
                    try
                        [i1,i2] = get_2_bases(X_OMP,Xhat_OMP,mod_threshold(jj),j);
                        Dic_ = [dcblker(Xhat_OMP(:,i1))/norm(dcblker(Xhat_OMP(:,i1)),2), dcblker(Xhat_OMP(:,i2))/norm(dcblker(Xhat_OMP(:,i2)),2), Dic];
                    catch
                        Dic_ = Dic;
                    end
                    DicCoh_ = Dic_'*Dic_;
                    tic;
                    [~, eomp_coeff, eomp_iopt, eomp_error,eomp_SSIM, eomp_hat]       = Enh_Convergence_OMP_pap7_(Dic_, Vector,sparse,y_init,1, err_threshold(ii));
                    [nocomb,E_SIZE_2,Approx_2,~] = runBRe(Vector,eomp_error,eomp_coeff,eomp_iopt,Dic_, Vector-eomp_hat(:,end), size(eomp_iopt,1), DicCoh_,bits_per_index,bits_per_coeff,0,eomp_hat(:,end),y_init);
                    %eomp_ci_matrix(eomp_iopt,kkk,ii,jj) = eomp_coeff;
                    kkk = kkk + 1;
                    EOMP_COEFF = [EOMP_COEFF;eomp_coeff];
                    EOMP_IND = [EOMP_IND;eomp_iopt];
...
function [i1,i2] = get_2_bases(y,y_hat, mod_,j)
fix_ind = [1:mod_:size(y_hat,2),size(y_hat,2)];
init_mat = zeros(size(y_hat,1),size(y_hat,2));
alpha_vector = zeros(size(y_hat,2),1);
%init_mat(:,fix_ind) = hat_mat(:,fix_ind);
