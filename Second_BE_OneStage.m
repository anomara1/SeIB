function [RESULTS] = Second_BE_OneStage(img,batchSize,sparse_,dicType) %#ok<*INUSD>
...
for i=1:1:floor(batchPerRow)
    for j=1:1:floor(batchPerCol)
        ...
        omp_runtime = toc;
        %tic;
        %[~, coeff, iopt,mp_ssim, y_i_mp]       = SSIM_MP_new(Dic, Vector,sparse);
        %mp_runtime = toc;
        %xxxxxx = 0;
        %tic;
        ...
        %[~, coeff, iopt,...
         %nOMP_curERR,omp_ssim,nomp_ssim, y_i_omp, y_i_nomp, nOMP_time]       = SSIM_OMP_nOMP_new_x(Dic, Vector,sparse);        nOMP_time = toc;
        ...
        %FBP_time = toc;
           ...
           %iter = iter+1;
           %temp_comp_ssim_avg = temp_comp_ssim./iter;
           %temp_comp_time = [mp_runtime somp_time];
           %xxx = [xxx y_i_omp(:,3)];
           %yyy = [yyy y_iOMP(:,3)];
           %zzz = [zzz y_i_nomp(:,3)];
           %sig = [sig Vector];
           ...
           %yyy_ = im2uint8(yyy);
           %zzz_ = im2uint8(zzz);
           % s1 = ssim(im2uint8(sig), xxx_);
           % s2 = ssim(im2uint8(sig), yyy_);
           % s3 = ssim(im2uint8(sig), zzz_);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for kk =1:1:sparse
         ...
         Delay_per_ite_locbe = [Delay_per_ite_locbe;  omp_t  omp_bm3d_t  miomp_t  miomp_bm3d_t]; 
         Delay_per_ite = [Delay_per_ite; omp_runtime/sparse iomp_runtime/sparse miomp_time/sparse  nOMP_time/sparse]; 
         delay_mean = mean(Delay_per_ite);
         Delay_per_ite_locbe_mean = mean(Delay_per_ite_locbe);
         %time_algs = [time_algs; omp_runtime iomp_runtime comp_time bm3d_runtime1];
         ROS(:,:,k) =  SS_omp_den;
         RON(:,:,k) =  NM_omp_den;
         RiS(:,k)   =  iomp_ssim;
         RiN(:,k)   =  iomp_curERR; 
